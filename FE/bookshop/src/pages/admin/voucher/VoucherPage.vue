<template>
  ...
  <div class="content__body">
    ...
    <VoucherDetail
      ...
      :formMode="formMode"
      :voucherIdSelected="voucherIdSelected"
    ></VoucherDetail>
...
import voucherColumns from "@/js/data/voucher";
export default {
  ...
  methods: {
    ...
    async loadData() {
      ...
      try {
        ...
        const res = await voucherService.getFilterPaging({ params });
        switch (res.status) {
          case 200:
     ...
     */
    onCreateItem() {
      this.voucherIdSelected = null;
      this.isShowForm = true;
      this.formMode = this.$Enum.FormMode.Add;
    },

    /**
     * Thực hiện hiển form khi click icon sửa
     * @param {string} id
     * @author LQHUY(19/03/2024)
     */
    onUpdateItem(id) {
      this.voucherIdSelected = id;
      this.isShowForm = true;
    ...
    updateListItemId(ids) {
      this.lstvoucherIdSelected = ids;
    },

    /**
     * Thực hiện hiển thị dialog xóa khi click icon xóa
     * @author LQHUY(19/03/2024)
     */
    ...
    onDeleteAll() {
      ...
      this.isShowDialogDelete = true;
      this.messageDialog =
        this.$Resource[this.$languageCode].ConfirmDeleteAll("Vourcher");
    },

    /**
     * Thực hiện xóa bản ghi theo typeDelete
     * @author LQHUY(19/03/2024)
     */
    ...
    async handleDeteleItem() {
      try {
        ...
        const res = await voucherService.delete(this.voucherIdSelected);
        switch (res.status) {
          case 200:
            this.$emitter.emit(
              "onShowToastMessage",
    ...
    async handleDeleteMany() {
      try {
        this.onHideDialogDelete();
        const res = await voucherService.deleteMany({
          data: this.lstvoucherIdSelected,
        });
        switch (res.status) {
          case 200:
  ...
  data() {
    return {
      ...
      voucherIdSelected: null,
      /**Lưu danh sách id các bản ghi được chọn */
      lstvoucherIdSelected: [],
