...
<script>
import userColumns from "@/js/data/user";
  ...
  methods: {
    ...
    async loadData() {
      ...
      try {
        ...
        const res = await userService.getFilterPagingByRoleName(userFilter);
        switch (res.status) {
          case 200:
    ...
    async handleDeteleItem() {
      try {
        ...
        const res = await userService.delete(this.userIdSelected);
        switch (res.status) {
          case 200:
            this.$emitter.emit(
              "onShowToastMessage",
    ...
    async handleDeleteMany() {
      try {
        this.onHideDialogDelete();
        const res = await userService.deleteMany({
          data: this.userIdsSelected,
        });
        switch (res.status) {
          case 200:
